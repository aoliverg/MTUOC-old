...
class Truecaser():
    ...
    def truecase(self, line, ucf=False, restoreCase=False, detokenize=True, keep_splitter=True):
        if self.tokenizer:
        ...
        for token in tokens:
            ...
            try:
                ...
                nsegment.append(token)
        if self.tokenizer and detokenize:
            nsegment=self.tokenizer.detokenize_s(" ".join(nsegment))     
        elif self.tokenizer and not detokenize:
            nsegment=" ".join(nsegment)
            if not keep_splitter:
                nsegment=nsegment.replace("▁","")
        else:
            nsegment=" ".join(nsegment)
        if ucf:

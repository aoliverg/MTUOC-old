...
def replaceNUM(segment,code="@NUM@"):
...
if corpus_GEN_MAX_READ==-1 and not from_scored_corpus_GEN:
    if corpus_GEN.endswith(".gz"):
        corpus_GEN_MAX_READ=rawincountgzipped(corpus_GEN)
    else:
        corpus_GEN_MAX_READ=rawincount(corpus_GEN)

    print("***************************************")
    print(corpus_GEN_MAX_READ)
    print("***************************************")
...
if preprocess_type=="smt":
    ...
    for linia in entrada:
        ...
        try:
            ...
            TLsegment=camps[1]
            if REPLACE_NUM:
                SLsegment=replaceNUM(SLsegment,NUM_CODE)
            sortidaSL.write(SLsegment+"\n")
            if REPLACE_NUM:
                TLsegment=replaceNUM(TLsegment,NUM_CODE)
    ...
    if GUIDED_ALIGNMENT:
        if VERBOSE:
            ...
            logfile.write(cadena+"\n")
        if DELETE_EXISTING:
            FILE="train.sp."+SLcode2+"."+TLcode2+".align" 
            if os.path.exists(FILE):
                os.remove(FILE)
        if ALIGNER=="fast_align":
            sys.path.append(MTUOC)
    ...
    if GUIDED_ALIGNMENT_VALID:
        ...
        if DELETE_EXISTING_VALID:
            FILE="val.sp."+SLcode2+"."+TLcode2+".align" 
            if os.path.exists(FILE):
                os.remove(FILE)
        if ALIGNER_VALID=="fast_align":
            sys.path.append(MTUOC)

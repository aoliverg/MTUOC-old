...
def translate_segment(segment):
    try:
        ...
        if not bos_annotate==None: segmentTAGS=bos_annotate+" "+segmentTAGS
        if not eos_annotate==None: segmentTAGS=segmentTAGS+" "+eos_annotate
        ...
        if MTUOCServer_restore_strategy=="mtengine":
            ...
            wordali=selectedalignment
            if sentencepiece:
                segmentWords=segmentPRENOTAGS.replace(" ","").replace(sp_joiner," ")
                selectedtranslationWords=selectedtranslationPre.replace(" ","").replace(sp_joiner," ")
        if not tokenizerSL==None:
        ...
        if MTUOCServer_restore_strategy=="eflomal":
        ...
        if hastags and MTUOCServer_restore_tags and MTUOCServer_restore_strategy=="mtengine":
            selectedtranslationTags=tagrestorer.restore_tags(segmentWordsTok, segmentTAGSTok, wordali, selectedtranslationWordsTok, spechar=sp_joiner)
        ...
        if not bos_annotate==None and selectedtranslationTags.startswith(bos_annotate+" "):selectedtranslationTags=selectedtranslationTags.replace(bos_annotate+" ","").strip()
        if not eos_annotate==None and selectedtranslationTags.endswith(" "+eos_annotate):selectedtranslationTags=selectedtranslationTags.replace(" "+eos_annotate,"").strip()
...
#restriction_vocabulary_SL=config["Preprocess"]["restriction_vocabulary_SL"]
tcmodel=config["Preprocess"]["tcmodel"]
bos_annotate=config["Preprocess"]["bos_annotate"]
if bos_annotate=="None": bos_annotate=None
eos_annotate=config["Preprocess"]["eos_annotate"]
if eos_annotate=="None": eos_annotate=None
